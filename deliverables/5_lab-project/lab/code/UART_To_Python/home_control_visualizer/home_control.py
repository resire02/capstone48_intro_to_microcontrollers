    ...
    try:
        ...
        while True:
            if uart.in_waiting:
                ...
                arduino_update_ui(received_data.lower())
...
def arduino_update_ui(text_input):
    # expecting: "temperature: 34C"
    if text_input.startswith('temperature:'):
        update_temperature(text_input[text_input.index(':')+1:].strip())
    # expecting: "room: 3 color: blue"
    elif text_input.startswith('room:'):
        color_index = text_input.index('color:')
        room = int(text_input[5:color_index].strip())
        color = text_input[color_index+6:].strip()
